...
module OctocatalogDiff
  module CatalogDiff
    class Filter
      # Filter out changes between an array of ints and an array of string representation of ints
      # E.g. Treat [8773, 8774, 8775] and ["8773", "8774", "8775"] the same
      class StringInt < OctocatalogDiff::CatalogDiff::Filter
        ...
        def build_results
          ...
          @diffs.each do |diff|
            next unless diff.old_value.is_a?(Array) && diff.new_value.is_a?(Array)
            next if diff.old_value.length != diff.new_value.length
            (0..diff.old_value.length).each do |i|
              @files_to_ignore.add diff.title if diff.old_value[i].to_s == diff.new_value[i].to_s
            end
          end
